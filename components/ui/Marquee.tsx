...
export default function Marquee({ items, className = '' }: MarqueeProps) {
    ...
    <div className="marquee-track flex shrink-0 items-center gap-10 pe-10">
      {items.map((item, i) => (
        <span key={i} className="flex items-center gap-10 whitespace-nowrap">
          <span className="font-anegra text-[22px] mobile:text-[18px] tracking-[1px] text-white/45">
            {item}
          </span>
  ...
  return (
    ...
    >
      {/* edge fades */}
      <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 mobile:w-12 bg-gradient-to-r from-background-dark to-transparent" />
      <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 mobile:w-12 bg-gradient-to-l from-background-dark to-transparent" />
